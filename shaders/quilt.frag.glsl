...
void main() {
    ...
    vec3 color = vec3(coord.x, coord.y, seed);
    ...
    // 1. Uncomment these lines to draw triangles
     ...
     vec2 loc = fract(squareCoord * 2.0);
     color = vec3(smoothstep(-1.0, 1.0, loc.y - loc.x - seed));

    // 2. Uncomment these lines to invert some of the triangles
     vec2 cell = squareCoord - loc;
     if (mod(2.0 * cell.x + cell.y + seed, 5.0 + seed) == 1.0) {
        color = 1.0  - color * seed * 20.0;
     }

    // 3. Uncomment these lines to produce interesting colors
     float c = mod(3.0 * cell.x + 2.0 * cell.y, 7.0) / 7.0;
     color = 1.0 - (1.0 - color) * vec3(seed * time, seed, time);
